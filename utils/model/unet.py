...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        drop_prob: float = 0.0,
        norm: str = "group",          # "group" | "instance" | "batch" | "none"
        num_groups: int = 8,
    ):
        """
        Args:
        ...
        self.down_sample_layers = nn.ModuleList([ConvBlock(in_chans, chans, drop_prob, norm=norm, num_groups=num_groups)])
        ch = chans
        for _ in range(num_pool_layers - 1):
            self.down_sample_layers.append(ConvBlock(ch, ch * 2, drop_prob, norm=norm, num_groups=num_groups))
            ch *= 2
        self.conv = ConvBlock(ch, ch * 2, drop_prob, norm=norm, num_groups=num_groups)

        self.up_conv = nn.ModuleList()
        self.up_transpose_conv = nn.ModuleList()
        for _ in range(num_pool_layers - 1):
            self.up_transpose_conv.append(TransposeConvBlock(ch * 2, ch))
            self.up_conv.append(ConvBlock(ch * 2, ch, drop_prob, norm=norm, num_groups=num_groups))
            ch //= 2

        self.up_transpose_conv.append(TransposeConvBlock(ch * 2, ch))
        self.up_conv.append(
            nn.Sequential(
                ConvBlock(ch * 2, ch, drop_prob, norm=norm, num_groups=num_groups),
...
def _make_norm(norm: str, num_channels: int, num_groups: int):
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_chans: int, out_chans: int, drop_prob: float, norm: str = "group", num_groups: int = 8):
        """
        Args:
        ...
        self.layers = nn.Sequential(
            ...
            _make_norm(norm, out_chans, num_groups),
...
class TransposeConvBlock(nn.Module):
    ...
    def __init__(self, in_chans: int, out_chans: int):
        ...
        self.layers = nn.Sequential(
            ...
            nn.GroupNorm(1, out_chans),  # transpose는 GN(1)=LN느낌으로 고정(안정)
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
